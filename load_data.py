...
def load_all_data_two_matrices():
    ...
    for i in range(0,len(filenames)-2,3):
        adjv = pd.read_csv(join(input_path,filenames[i]))
        features = pd.read_csv(join(input_path,filenames[i+1]))
        adjs = pd.read_csv(join(input_path,filenames[i+2]))
       ...
       # features["mean_ratio"]= np.maximum(features["mean_ratio"],1000)
        ...
        features = pipe.fit_transform(features)
        Av = np.array(adjv, dtype = "int32")[:,1:]
        As = np.array(adjs, dtype = "int32")[:,1:]
        X = np.array(features)

        data.append((Av,As,X,y))
        feat_size = X.shape[-1]
        if Av.shape[0]> max_graph_size:
            max_graph_size = Av.shape[0]
    ...
    for datum in data:
        ...
        deg_mat_inv_half_v = np.zeros(Av.shape)
        for j in range(len(Av)):
            deg_mat_inv_half_v[j][j] = sum(Av[j])**(-0.5)

        Av = np.matmul(deg_mat_inv_half_v,Av)
        Av = np.matmul(Av,deg_mat_inv_half_v) 

        Avbig = np.zeros((max_graph_size,max_graph_size))
        Avbig = np.identity(max_graph_size)
        Avbig[0:Av.shape[0],0:Av.shape[1]] = Av
        #--------------------------------------
        deg_mat_inv_half_s = np.zeros(As.shape)
        for j in range(len(As)):
            deg_mat_inv_half_s[j][j] = sum(As[j])**(-0.5)

        As = np.matmul(deg_mat_inv_half_s,As)
        As = np.matmul(As,deg_mat_inv_half_s) 

        Asbig = np.zeros((max_graph_size,max_graph_size))
        Asbig = np.identity(max_graph_size)
        Asbig[0:As.shape[0],0:As.shape[1]] = As
        #----------------------------------------
        ...
        ybig = -1*np.ones((max_graph_size,))
        ybig[0:y.shape[0]] = y
        fixed_data.append((Avbig,Asbig,Xbig,ybig))
