single_vessel_plotter <- function(vessel_coords, centered = TRUE, frenet = FALSE, scale = 1, frenet_vectors, col_metric = NULL, col = NULL, ...){
  ...
  if(centered){
    tran_x <- 0.5*sum(range(vessel_coords[,1], na.rm = T))
    tran_y <- 0.5*sum(range(vessel_coords[,2], na.rm = T))
    tran_z <- 0.5*sum(range(vessel_coords[,3], na.rm = T))
  ...
  x_range <- range(vessel_coords[,1], na.rm = T)[2] - range(vessel_coords[,1], na.rm = T)[1]
  y_range <- range(vessel_coords[,2], na.rm = T)[2] - range(vessel_coords[,2], na.rm = T)[1]
  z_range <- range(vessel_coords[,3], na.rm = T)[2] - range(vessel_coords[,3], na.rm = T)[1]
