dat_file_reader <- function(dat_filename, scale = TRUE){
  ...
  if(scale){
    ...
    xfactor <- 1#as.numeric(1, value = TRUE)[[1]][1])/1000
    yfactor <- 1#as.numeric(1, value = TRUE), "x")[[1]][2])/1000
    zfactor <- 1#as.numeric(1, value = TRUE), "x")[[1]][3])/1000
